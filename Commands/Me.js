...
exports.run = async (client, message, args) => {
    ...
    messageList = messageList.map((md, index)=> `\`${index + 1}.\` ${client.channels.cache.has(md.Id) ? client.channels.cache.get(md.Id).toString() : "#channel"}: \`${md.Total} mesaj\``).join("\n║");
    let embed = new Discord.MessageEmbed();
      embed.setColor('#00ffd0')
    .setFooter('Kral Ses Aktiflik')
